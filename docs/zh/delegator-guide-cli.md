...
`barkiscli`: 与`barkisd`全节点交互的命令行用户界面。 

::: 安全提示

**请检查并且确认你下载的`barkiscli`是可获得的最新稳定版本**
:::
...
`barkiscli` 需要通过操作系统的终端窗口使用，打开步骤如下所示： 
...
前，您需要先安装`barkiscli`，同时，您需要知道你希望在哪里保存和使用您的私钥。 最好的办法是把他们保存在一台没有上网的电脑或者一个硬件钱包设备里面。 将私钥保存在一台联网的电脑里面比较危险，任何人通过网络攻击都有可能获取您的私钥，进而盗取您的资产。 
...
// 样例: barkiscli config node https://77.87.106.33:26657
```
...
```bash
barkiscli config chain-id gos-6
```
...
`barkiscli` 可以帮助您获得所有区块链的相关信息， 比如账户余额，抵押通证数量，奖励，治理提案以及其他信息。下面是一组用于委托操作的命令
...
barkiscli tx gov vote <proposalID> <option> --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
```bash
barkiscli tx sign unsignedTx.json --from-addr <delegatorAddr>> signedTx.json
```
