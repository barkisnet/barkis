...
::: 提示
如果没有指定，`consensus_pubkey`将默认为`barkisd tendermint show-validator`命令的输出。`key_name`是将用于对交易进行签名的私钥的名称。
:::
...
如果被因为掉线而遭到jail，你可以重获你的投票股权以重回验证人队伍。首先，如果`barkisd`没有运行，请再次启动：
...
### 问题 #2 : 我的`barkisd`由于`too many open files`而崩溃

Linux可以打开的默认文件数（每个进程）是1024。已知`barkisd`可以打开超过1024个文件。这会导致进程崩溃。快速修复运行`ulimit -n 4096`（增加允许的打开文件数）来快速修复，然后使用`barkisd start`重新启动进程。如果你使用`systemd`或其他进程管理器来启动`barkisd`，则可能需要在该级别进行一些配置。解决此问题的示例`systemd`文件如下：
