...
This document explains how the genesis file of the BarkisNet mainnet is structured. It also explains how you can build a genesis file for your own `barkis` testnet.
...
The genesis file is stored in `~/.barkisd/config/genesis.toml`.
...
```json
"chain_id": "barkisnet-2",
```

## Consensus Parameters

Next, the genesis file defines consensus parameters. Consensus parameters regroup all the parameters that are related to the consensus layer, which is `Tendermint` in the case of `barkis`. Let us look at these parameters:
...
```bash
// Example: barkisd add-genesis-account barkis1qs8tnw2t8l6amtzvdemnnsq9dzk0ag0z37gh3h 10000000ubarkis
...
"accounts": [
      {
        "address": "barkis1qs8tnw2t8l6amtzvdemnnsq9dzk0ag0z37gh3h",
        "coins": [
          {
            "denom": "ubarkis",
            "amount": "10000000"
...
- `original_vesting`: Vesting is natively supported by `barkis`. You can define an amount of token owned by the account that needs to be vested for a period of time before they can be transferred. Vested tokens can be delegated. Default value is `null`.
...
- `pool`
    + `not_bonded_tokens`: Defines the amount of tokens not bonded (i.e. delegated) in genesis. Generally, it equals the total supply of the staking token (`ubarkis` in this example).
...
This command will add all the `gentxs` stored in `~/.barkisd/config/gentx` to the genesis file. In order to create a genesis transaction, click [here](./validators/validator-setup.md#participate-in-genesis-as-a-validator).
