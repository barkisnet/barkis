## Build Barkis Deterministically

Gitian is the deterministic build process that is used to build the Barkis executables. It provides a way to be reasonably sure that the executables are really built from the git source. It also makes sure that the same, tested dependencies are used and statically built into the executable.
...
This page contains all instructions required to build and sign reproducible Barkis binaries for Linux, Mac OS X, and Windows.
...
* `barkis.sigs` containing the signed build report.
...
```
cd barkis/ && ./contrib/gitian-build.sh -s user@example.com all
```
...
At the end of the procedure, build results can be found in the `./barkis.sigs` directory:

Please refer to the `contrib/gitian-build.sh`'s help screen for further information on its usage.

## Signatures upload

Once signatures are generated, they could be uploaded to barkis's dedicated repository: https://github.com/barkisnet/barkis.sigs.
