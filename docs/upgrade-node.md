...
This document describes the upgrade procedure of a `barkisd` full-node to a new version. 

## Software Upgrade

First, stop your instance of `barkisd`. Next, upgrade the software:
...
```bash
cd $HOME/.barkisd/config
barkisd export --for-zero-height --height=<export-height> > new_genesis.json
```
...
::: warning 
If you are running a **validator node** on the mainnet, always be careful when doing `barkisd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
