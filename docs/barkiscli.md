...
```bash
barkiscli tx send <sender_key_name_or_address> <recipient_address> 10ubarkis \
  --chain-id=<chain_id>
```
...
```bash
barkiscli tx send <sender_key_name_or_address> <destination_barkisaccaddr> 10ubarkis \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
barkiscli tx send <sender_address> <recipient_address> 10ubarkis \
  --chain-id=<chain_id> \
...
Governance is the process from which users in the Barkis Net can come to consensus
