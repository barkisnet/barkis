...
chain_id = "barkis-11001"
reconnect = true # true is the default
secret_key = "~/.tmkms/secret_connection.key"

[[providers.ledgertm]]
chain_ids = ["barkis-11001"]
```

- Edit `addr` to point to your `barkisd` instance.
- Adjust `chain-id` to match your `.barkisd/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `barkisd`.
...
## Barkis configuration

You need to enable KMS access by editing `.barkisd/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `barkisd`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell barkisd the key that we've got in the previous section.

```bash
barkisd gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `barkisd`. You should see that the KMS connects and receives a signature request.
