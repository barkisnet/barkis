...
- **Tendermint Key**: This is a unique key used to sign consensus votes. 
    + It is associated with a public key `cosmosvalconspub` (Get this value with `barkisd tendermint show-validator`)
    + It is generated when the node is created with barkisd init.
    
- **Application key**: This key is created from `barkiscli` and used to sign transactions. Application keys are associated with a public key prefixed by `cosmospub` and an address prefixed by `cosmos`. Both are derived from account keys generated by `barkiscli keys add`.
